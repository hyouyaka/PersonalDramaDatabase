...
- 漫播确认下架后执行 info/watchcount 归档；猫耳自动归档代码保留但当前停用
- 不更新 SQLite
...
- 适合猫耳触发 `418` 后断点继续
- 猫耳 HTTP 403、漫播响应 `code=400` 且 `msg=作品已下架` 时按 30/60/120 秒延迟重试；包含首次请求共 4 次
- 猫耳连续 4 次 HTTP 403 时保留活跃 info，并把本次 latest `view_count` 写为 `null`；不写入当日 history 点，避免恢复后产生虚假增量
- 猫耳自动 archive 由 `ARCHIVE_ENABLED["missevan"]` 暂时关闭；漫播连续 4 次明确下架响应仍正常归档
- 命中这些状态的作品进入延迟队列，首轮继续抓取其他作品
...
- CV 榜遇到显式 `view_count: null` 时保留该剧，并按播放量 0 计入本次汇总；完全缺失 watchcount 条目的剧目仍跳过
